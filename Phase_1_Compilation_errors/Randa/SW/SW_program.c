...
void SW_Init(u8 Port, u8 Pin, GPIO_ResType_t ResType)
{
    GPIO_PadConfig_t padConfig = {
		...
		This method in initializing struct gives a compilation error in keil4 since all struct values must have initial constants
		using a more safe way to do is more efficient 
		therefore I recommend changing it to:
		GPIO_PadConfig_t padConfig = {
        GPIO_PULL_DOWN,
        GPIO_UNLOCKED,
        GPIO_SLEW_RATE_DISABLE
...
void EXTSW_Init(u8 Port, u8 Pin)
{
    GPIO_PadConfig_t padConfig = {
		...
		This method in initializing struct gives a compilation error in keil4
		using a more safe way to do is more efficient 
		therefore I recommend changing it to:
		GPIO_PadConfig_t padConfig = {
        GPIO_PULL_DOWN,  //pull down for external
