...
/*******************************************ERROR (1) FIXED***********************************************************/
	/*
		"CLCD_voidSendCommand()" --> This function is never declared in any header file nor implemented in the .c file
		This should be modified according to your code logic however I assumed it is mapped to the function named 
		"CLCD_8DataPinssendCommand( u8 command)" in the private file.
		
		You should check my theory is valid.
		This is the code before"
		
				Systick_DelayIn_ms(1);
        CLCD_voidSendCommand(0x38);
        Systick_DelayIn_ms(1);
        CLCD_voidSendCommand(0xC);
        Systick_DelayIn_ms(1);
        CLCD_voidSendCommand(0x1);
        Systick_DelayIn_ms(1);
				
			// however after modifying the binary to hexdecimal
...
/*******************************************ERROR (2) FIXED***********************************************************/
	/*
		ALL Binary values were changed to hexadecimal 
		since keil4 gives an error for binary and hexadecimal is more common for embedded systems
	Here is the changes:
...
void CLCD_voidSendSpecialCharacter(u8 Copy_u8BlockNum, u8 Copy_HorizontalValue,u8 copy_AbsVerticalValue,u8 *Pu8ArrayPattern){
    ...
    for (i = 0; i < 8; i++)
    {
		...
		/*******************************************ERROR FIXED***********************************************************/
	/*
		Some minor keil4 compilation error:
		cade before:
		for (u8 i = 0; i < 8; i++){
        CLCD_8DATASendData(Pu8ArrayPattern[i]);
    }
		Plus you can't put a space between function : "CLCD_8DATASendData" & its arguments "(Pu8ArrayPattern[i])"
		
		Adding to the an excutable function must be put between the declaration of "i" and its use in the for loop
		Keil4 protocol demands that
